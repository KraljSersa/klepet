...
function obdelajDregljaj(socket) {
  socket.on('dregljaj', function (dregljaj) {
    ...
    // dodaj funkcijo da ne moreš dregati samega sebe
    if (dregljaj.vzdevek) {
      var socketIdNaslovnika = null;
      for (var id in vzdevkiGledeNaSocket) {
      ...
      if (vzdevkiGledeNaSocket[socket.id] == vzdevkiGledeNaSocket[socketIdNaslovnika]) {
        io.sockets.sockets[socketIdNaslovnika].emit('sporocilo', { besedilo:"Ne moreš dregniti samega sebe!" });
      }
